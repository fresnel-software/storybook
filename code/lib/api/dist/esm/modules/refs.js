...
import { transformSetStoriesStoryDataToStoriesHash, transformStoryIndexToStoriesHash } from '../lib/stories';
const {
  location,
  fetch
} = global;
...
export const init = ({
  ...
  const api = {
    ...
    setRef: (id, _ref, ready = false) => {
      ...
      if (setStoriesData) {
        storiesHash = transformSetStoriesStoryDataToStoriesHash(map(setStoriesData, ref, {
          storyMapper
        }), {
          provider,
          docsOptions
        ...
        storiesHash = transformStoryIndexToStoriesHash(storyIndex, {
          provider,
          docsOptions
        });
      }
