...
class ArgsStore {
  ...
  setInitial(story) {
    if (!this.initialArgsByStoryId[story.id]) {
      ...
      const delta = (0, _args.deepDiff)(this.initialArgsByStoryId[story.id], this.argsByStoryId[story.id]);
      this.initialArgsByStoryId[story.id] = story.initialArgs;
      this.argsByStoryId[story.id] = story.initialArgs;
  ...
  updateFromDelta(story, delta) {
    ...
    const validatedDelta = (0, _args.validateOptions)(delta, story.argTypes); // NOTE: we use `combineArgs` here rather than `combineParameters` because changes to arg
  ...
  updateFromPersisted(story, persisted) {
    ...
    const mappedPersisted = (0, _args.mapArgsToTypes)(persisted, story.argTypes);
