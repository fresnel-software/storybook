...
const init = ({
  ...
  const api = {
    ...
    handleKeydownEvent(event) {
      const shortcut = (0, _shortcut.eventToShortcut)(event);
      const shortcuts = api.getShortcutKeys();
      const actions = keys(shortcuts);
      const matchedFeature = actions.find(feature => (0, _shortcut.shortcutMatchesShortcut)(shortcut, shortcuts[feature]));
  ...
  const initModule = () => {
    ...
    fullAPI.on(_coreEvents.PREVIEW_KEYDOWN, data => {
