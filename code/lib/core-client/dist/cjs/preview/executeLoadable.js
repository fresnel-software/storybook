...
function executeLoadable(loadable) {
  ...
  if (reqs) {
    reqs.forEach(req => {
      req.keys().forEach(filename => {
        try {
          ...
          _clientLogger.logger.error(`Unexpected error while loading ${filename}: ${errorString}`);
        }
      });
    });
  } else {
    ...
    if (Array.isArray(exported) && exported.every(obj => obj.default != null)) {
      ...
      _clientLogger.logger.warn(`Loader function passed to 'configure' should return void or an array of module exports that all contain a 'default' export. Received: ${JSON.stringify(exported)}`);
    }
  }
